...
class ApiService {
  ...
  //* Log in user, store token securely and update header
  ...
  //* Log out user, remove token from storage and header
  ...
  //* Register new user, store token securely and update header
  ...
  /// Upload a photo file and return its URL
  ...
  //* Fetch a page of movies with pagination
  Future<List<MovieDTO>?> fetchMoviePage({required BuildContext context, int page = 1}) async {
    try {
      ...
      if (resp.statusCode != 200) {
        throw DioException(requestOptions: resp.requestOptions, response: resp, error: 'Server did not return page $page (status ${resp.statusCode})');
      }
      final data = (resp.data as Map<String, dynamic>)['data'] as Map<String, dynamic>;
