...
class _ProfilephotoScreenState extends State<ProfilephotoScreen> {
  ...
  Future<void> _pickImageFromGallery() async {
    ...
    if (picked != null) {
      try {
        String resUrl = await ApiService.instance.uploadPhoto(context: context, file: File(picked.path)) ?? '';
  ...
  Widget profilePhotoBody() {
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 20.0), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [const SizedBox(height: 40), const Text('Fotoğraflarınızı Yükleyin', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)), const SizedBox(height: 8), const Text('Resources out incentivize\nrelaxation floor loss cc.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white70, fontSize: 14)), const SizedBox(height: 40), _photoImportArea(), const Spacer(), _continueButton(), const SizedBox(height: 20)]));
  }

  Widget _photoImportArea() {
    return Center(
