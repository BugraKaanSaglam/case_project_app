...
class LimitedOfferBottomSheetState extends State<LimitedOfferBottomSheet> {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Container(
        decoration: BoxDecoration(color: const Color(0xFF1E1E1E), borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
        padding: EdgeInsets.symmetric(horizontal: 16, vertical: 24),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('sinirli_teklif'.tr(), style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
            SizedBox(height: 8),
            Text('limited_offer_desc'.tr(), style: TextStyle(color: Colors.white70), textAlign: TextAlign.center),
            SizedBox(height: 24),
            Text('alacaginiz_bonuslar'.tr(), style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
            SizedBox(height: 16),
            Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [_BonusItem(icon: Icons.diamond, label: 'premium_hesap'), _BonusItem(icon: Icons.swap_horiz, label: 'daha_fazla_eslesme'), _BonusItem(icon: Icons.arrow_upward, label: 'one_cikarma'), _BonusItem(icon: Icons.favorite, label: 'daha_fazla_begeni')]),
            SizedBox(height: 24),
            Text('kilidi_acmak_icin'.tr(), style: TextStyle(color: Colors.white)),
            SizedBox(height: 16),
            ...
            SizedBox(height: 24),
            SizedBox(width: double.infinity, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.redAccent, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), padding: EdgeInsets.symmetric(vertical: 16)), onPressed: widget.onViewAll, child: Text('tum_jetonlari_gor'.tr(), style: TextStyle(fontSize: 16, color: Colors.white)))),
          ],
        ),
      ),
    );
...
class _BonusItem extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Column(children: [Container(width: 48, height: 48, decoration: BoxDecoration(color: Colors.black54, shape: BoxShape.circle), child: Icon(icon, color: Colors.pinkAccent)), SizedBox(height: 8), Text(label.tr(), style: TextStyle(color: Colors.white70, fontSize: 12), textAlign: TextAlign.center)]);
  }
}
